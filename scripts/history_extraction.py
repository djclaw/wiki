...
def load_recent_entries(days: int = 7) -> list[dict[str, Any]]:
    ...
    for line in lines:
        ...
        entries.append({'timestamp': dt.isoformat(timespec='minutes'), 'text': line})
    if not entries:
        return []
...
- Keep only high-value, page-worthy candidates.
- Prefer 1 to 4 nodes, 0 to 1 relations, and 1 event per entry.
- Preserve the entry timestamp for events unless the text clearly implies a different time.
- Default to retaining only stable named entities that would still make sense as standalone wiki pages a month later.
- Strongly prefer: named places (cities, states, venues), named products/tools/services, named projects/repos, named organizations/platforms, and clearly identified people.
- Avoid abstract concepts, vague work themes, meta phrases, workflow labels, generic containers, OS/device categories, and descriptive headings unless they are the central long-lived entity.
...
- When unsure between keeping and dropping a node, drop it.
...
Preferred node types in practice: place, project, tool, org, person
...
def get_provider_api_key(provider: str) -> str:
    env_map = {'gemini': ['GEMINI_API_KEY', 'GOOGLE_API_KEY'], 'groq': ['GROQ_API_KEY'], 'openai': ['OPENAI_API_KEY']}
...
def _normalize_label(label: str, node_type: str) -> str:
    ...
    low = s.lower()
    mapping = {
        'djclaw post': 'DJClaw', 'djclaw posts': 'DJClaw', 'dj claw': 'DJClaw',
        'github pages': 'GitHub Pages', 'github page': 'GitHub Pages', 'github repo': 'GitHub',
        'github repository': 'GitHub', 'github issue': 'GitHub Issues', 'github issues': 'GitHub Issues',
        'djclaw.github.io': 'DJClaw', 'djclaw site': 'DJClaw', 'djclaw website': 'DJClaw', 'djclaw blog': 'DJClaw',
        'trips project': 'trips', 'trip project': 'trips', 'new orleans trip page': 'New Orleans', 'new orleans page': 'New Orleans',
        'personal wiki': 'wiki', 'wiki project': 'wiki', 'wiki repo': 'wiki', 'djclaw/wiki': 'wiki',
        'history.md': 'HISTORY.md', 'history md': 'HISTORY.md', 'open street map': 'OpenStreetMap', 'osm': 'OpenStreetMap', 'gh': 'GitHub',
        'readme': '', 'wiki readme': '', 'extraction pipeline': '', 'structured extraction': '', 'recent history sample': '', 'structured candidates': '',
    }
    if low in mapping:
        return mapping[low]
    if low == 'djclaw': return 'DJClaw'
    if low in {'github', 'github.com'}: return 'GitHub'
    if low == 'logseq': return 'Logseq'
    if low == 'seqlog': return 'SeqLog'
    if low == 'new orleans': return 'New Orleans'
    if low == 'airbnb': return 'Airbnb'
    if low == 'leaflet': return 'Leaflet'
    if low in {'openstreetmap', 'osm'}: return 'OpenStreetMap'
    if low in {'wiki', 'personal wiki'}: return 'wiki'
    if low in {'trips', 'trips repo'}: return 'trips'
    return s
...
def _keep_node(label: str, node_type: str) -> bool:
    ...
    if not s:
        return False
    preferred_types = {'place', 'project', 'tool', 'org', 'person'}
    hard_block = {'user', 'assistant', 'article', 'post', 'file', 'page', 'repo', 'script', 'project', 'tool', 'doc', 'entry', 'markdown file', 'website', 'blog', 'platform', 'system', 'process', 'content', 'readme', 'history sample', 'recent history sample', 'structured candidates', 'structured extraction', 'extraction pipeline'}
    if low in hard_block:
        return False
    if re.fullmatch(r'[0-9a-f]{7,40}', low) or re.fullmatch(r'v?\d+(?:\.\d+){0,3}', low):
        return False
    if s.endswith(('.html', '.md', '.json')) or re.search(r'/home/|\.py$|\.sh$|\.js$', s):
        return False
    weak_abstract = {'update', 'status', 'plan', 'note', 'notes', 'design', 'feature', 'comparison', 'listing', 'confirmed', 'explained', 'explainer', 'refine', 'personal', 'current', 'latest', 'recent', 'future', 'past', 'next', 'implementation', 'planning', 'workflow', 'pipeline', 'architecture', 'system', 'mode', 'context', 'logic', 'structure', 'layout', 'mobile', 'desktop', 'style', 'theme', 'interface', 'interaction', 'model'}
    if node_type in {'topic', 'other'} and low in weak_abstract:
        return False
    protected = {'wiki', 'DJClaw', 'GitHub', 'GitHub Pages', 'Logseq', 'SeqLog', 'OpenStreetMap', 'Leaflet', 'Airbnb', 'New Orleans', 'trips', 'New York', 'New Jersey', 'Javits Center'}
    if s in protected:
        return True
    if low in {'macos', 'ios', 'android', 'iphone', 'ipad', 'linux', 'raspberry pi', 'telegram', 'wechat', 'nanobot', 'macbook', 'google cloud', 'moonshotai/kimi-k2-instruct', 'kimi-k2-instruct', 'llama-3.3-70b-versatile', 'gpt-4o-mini', 'baml', 'dspy', 'instructor', 'llamaindex', 'graphrag'}:
        return False
...
def parse_response(raw: str) -> tuple[list[NodeCandidate], list[RelationCandidate], list[EventCandidate]]:
    ...
    for item in data.get('nodes', []):
        ...
        if item['label'] == 'DJClaw': item['type'] = 'project'
        if item['label'] in {'GitHub', 'GitHub Pages', 'GitHub Issues', 'Airbnb'}: item['type'] = 'org'
        if item['label'] in {'New Orleans'}: item['type'] = 'place'
        if item['label'] in {'Logseq', 'SeqLog', 'Leaflet', 'OpenStreetMap'}: item['type'] = 'tool'
    ...
    for item in data.get('relations', []):
        s = item.get('source_label', '').strip()
        t = item.get('target_label', '').strip()
        rt = (item.get('relation_type') or '').strip()
        conf = float(item.get('confidence') or 0)
        st = node_type_by_label.get(s, 'other')
        tt = node_type_by_label.get(t, 'other')
        if s not in allowed_labels or t not in allowed_labels or s == t:
            continue
        if rt in {'mentioned_with', 'related_to', 'about', 'located_in', 'other'}:
            continue
        if rt == 'works_on' and (conf < 0.85 or st not in {'person', 'org'}):
            continue
        if rt == 'uses' and (conf < 0.85 or st not in {'person', 'org', 'project'}):
            continue
        if rt == 'published_to' and (conf < 0.9 or tt not in {'org', 'project'}):
            continue
        rel_rows.append(item)
    rel_rows = _dedupe_dict_rows(rel_rows, ('source_label', 'relation_type', 'target_label'))[:1]
    event_rows = []
    ...
    return [NodeCandidate(**item) for item in node_rows], [RelationCandidate(**item) for item in rel_rows], [EventCandidate(**item) for item in event_rows]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument('--provider', choices=['gemini', 'groq', 'openai'], default=os.getenv('WIKI_EXTRACTION_PROVIDER', 'groq'))
...
def choose_default_model(provider: str) -> str:
    defaults = {'gemini': os.getenv('WIKI_EXTRACTION_GEMINI_MODEL', 'gemini-2.5-flash'), 'groq': os.getenv('WIKI_EXTRACTION_GROQ_MODEL', 'moonshotai/kimi-k2-instruct'), 'openai': os.getenv('WIKI_EXTRACTION_OPENAI_MODEL', 'gpt-4o-mini')}
    return defaults[provider]


def run_structured_extraction(provider: str, model: str, days: int, limit: int) -> list[dict[str, Any]]:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    entries = load_recent_entries(days=days)
    ...
    for i, entry in enumerate(entries, start=1):
        evidence = EvidenceRecord(evidence_id=f'evidence-{i:03d}', source_id='history_md', timestamp=entry['timestamp'], text=entry['text'])
        item = EntryExtraction(entry_id=f'entry-{i:03d}', timestamp=entry['timestamp'], evidence=evidence)
        try:
            raw = try_model_call(build_prompt(entry), provider=provider, model=model)
            item.raw_response = raw
            item.nodes, item.relations, item.events = parse_response(raw)
        except Exception as e:
            item.parse_error = str(e)
        results.append({'entry_id': item.entry_id, 'timestamp': item.timestamp, 'evidence': asdict(item.evidence), 'nodes': [asdict(x) for x in item.nodes], 'relations': [asdict(x) for x in item.relations], 'events': [asdict(x) for x in item.events], 'raw_response': item.raw_response, 'parse_error': item.parse_error})
        print(f'[{i}/{len(entries)}] {item.timestamp} ' + ('ok' if not item.parse_error else f'error={item.parse_error[:120]}'))
...
def main() -> int:
    ...
    run_structured_extraction(provider=provider, model=model, days=args.days, limit=args.limit)
    return 0
