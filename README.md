# Personal Wiki

A lightweight, static personal Wikipedia powered by a local knowledge graph.

## Structure
- `index.html` — entry index + search
- `entries/` — entity pages (one per entry)
- `data/search-index.json` — search index for the front-end
- `scripts/build_search_index.py` — generates the search index
- `style.css` — minimal Wikipedia-like theme
- `templates/` — base HTML templates

## Add a New Entry
1) Create a new file under `entries/` (e.g. `entries/your-entity.html`).
2) Use the existing entry pages as a template.
3) Optional metadata (for search):
   - `data-tags="a,b,c"`
   - `data-categories="x,y"`
   - `data-aliases="alt name,short"`
   - `data-related="other entity,another"`
   - `data-lat="40.7128"` / `data-lon="-74.0060"`
4) Regenerate the index:
   ```bash
   python3 scripts/build_search_index.py
   ```

## UI Language
- UI text is controlled by `ui-text.json`.
- Toggle with `?lang=en` or `?lang=zh` (the choice is saved in localStorage).
- Entry content stays in its original language.

## Related Entries / Backlinks
- `data-related` on an entry drives the "Related" list.
- The "Linked from" list is generated by scanning other entries' `data-related` fields.

## Local DB (MVP3)
- Local SQLite DB path: `/home/dj/.nanobot/workspace/memory/wiki.db`
- DB is not tracked by git.

## Sources (MVP3)
- Source configuration: `config/sources.json`
- Currently enabled: `HISTORY.md`
- Placeholders: notes dir / Notion export / custom CSV

## Extraction (MVP3)
1) Extract from HISTORY.md:
   ```bash
   python3 scripts/extract_history.py
   ```
2) Build local SQLite DB:
   ```bash
   python3 scripts/build_db.py
   ```

## Publishing
This repo is intended to be published via GitHub Pages (main branch / root).
